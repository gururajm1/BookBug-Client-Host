...
const BookDetails = () => {
      ...
      try {
        ...
        if (isAddedPath) {
          ...
          const userResponse = await axios.get(
            `https://bookbug-server-host.onrender.com/api/users/${email}`
          );
  ...
  const handleDelete = async () => {
    try {
      const email = localStorage.getItem("book-bug");
      await axios.delete(
        "https://bookbug-server-host.onrender.com/api/delete-book",
        {
          data: { email, isbn13 },
        }
      );
  ...
  const handleUpdate = async () => {
    try {
      ...
      const url = isAddedPath
        ? "https://bookbug-server-host.onrender.com/api/added-update"
        : "https://bookbug-server-host.onrender.com/api/update-book";
